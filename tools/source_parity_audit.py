#!/usr/bin/env python3
"""Open-Meteo vs Weather Underground source parity audit (LOG_ONLY).

Standalone source-parity dossier for one city/ICAO pair. It never imports
bot.py, never writes runtime state, and does not scrape Weather Underground.
If no manual or future reliable WU dataset is provided, the audit reports
WU_FETCHER_MISSING. Gamma-derived exact settlement is optional read-only
triage and does not replace a WU fetcher.
"""
...
def fetch_open_meteo_daily(start: date, end: date, lat: float, lon: float, tz: str) -> dict[str, float]:
...
def build_exact_slug(city: str, date_local: str, strike_c: float | int) -> str:
    month, day, year = date_to_slug_parts(date_local)
    strike = int(float(strike_c))
    return f"highest-temperature-in-{slugify_city(city)}-on-{month}-{day}-{year}-{strike}c"


def parse_exact_slug(slug: str, city: str) -> dict | None:
    match = re.fullmatch(
        r"highest-temperature-in-([a-z0-9-]+)-on-([a-z]+)-(\d{1,2})-(\d{4})-(\d+)c",
        str(slug or "").strip().lower(),
    )
    if not match:
        return None
    if match.group(1) != slugify_city(city):
        return None
...
def extract_gamma_exact_result(market: dict, config: AuditConfig, fallback_slug: str | None = None) -> dict:
    slug = str(market.get("slug") or fallback_slug or "")
    parsed = parse_exact_slug(slug, config.city)
    if not parsed:
        raise ValueError(f"Gamma market slug is not a {config.city} exact-temperature slug: {slug}")
    source = str(market.get("resolutionSource") or "")
    if "wunderground.com" not in source.lower() or config.icao.lower() not in source.lower():
        raise ValueError(f"Gamma market source is not WU/{config.icao}: {source}")
...
def infer_settlement_from_exact_markets(markets: list[dict], config: AuditConfig) -> tuple[dict[str, dict], list[str]]:
    ...
    for market in markets:
        try:
            result = extract_gamma_exact_result(market, config)
...
def collect_gamma_markets_for_blocked_rows(
    ...
    for date_local, strikes in sorted(by_date.items()):
        ...
        for strike in range(low, high + 1):
            slug = build_exact_slug(config.city, date_local, strike)
...
def build_gamma_settlement_comparisons(
    ...
    markets, errors = collect_gamma_markets_for_blocked_rows(
        blocked_rows,
        config,
        fetcher=fetcher,
        neighbor_radius=neighbor_radius,
    )
    inferred, warnings = infer_settlement_from_exact_markets(markets, config)
...
def decide_gamma_settlement_verdict(metrics: dict, errors: list[dict], warnings: list[str]) -> tuple[str, list[str]]:
    ...
    if n_compared == 0:
        ...
        return "INSUFFICIENT_DATA", reasons or ["no comparable Gamma-derived settlement dates"]
    if n_compared < OPUS_MIN_GAMMA_N:
        reasons.append(f"n_dates_compared={n_compared} < {OPUS_MIN_GAMMA_N}")
    ...
    if max_abs is None or max_abs > OPUS_MAX_ABS_DELTA_C:
        reasons.append(f"max_abs_delta_c={max_abs} > {OPUS_MAX_ABS_DELTA_C}")
    metric_failure = any(reason.startswith(("median_abs_delta_c", "pct_abs_delta_ge_1c", "max_abs_delta_c")) for reason in reasons)
    if metric_failure and n_compared >= OPUS_MIN_GAMMA_FAIL_N:
...
def decide_verdict(metrics: dict, wu_status: str, blocked_metrics: dict) -> tuple[str, list[str]]:
    ...
    if wu_status == "missing_fetcher":
        return "WU_FETCHER_MISSING", ["no reliable WU fetcher exists in repo and no --wu-csv was provided"]
    n = metrics.get("n_compared") or 0
    if n == 0:
        return "INSUFFICIENT_DATA", ["no overlapping Open-Meteo/WU daily rows"]
    if n < OPUS_MIN_WU_N:
        reasons.append(f"n_compared={n} < {OPUS_MIN_WU_N}")
    median_abs = metrics.get("median_abs_delta_c")
    if median_abs is None or median_abs > OPUS_MAX_MEDIAN_ABS_DELTA_C:
        reasons.append(f"median_abs_delta_c={median_abs} > {OPUS_MAX_MEDIAN_ABS_DELTA_C}")
    pct_ge_1 = metrics.get("pct_abs_delta_ge_1c")
    ...
    if reasons:
        metric_fail = any(reason.startswith(("median_abs_delta_c", "pct_abs_delta_ge_1c", "max_abs_delta_c")) for reason in reasons)
        return "SETTLEMENT_GAMMA_PARITY_FAIL" if metric_fail else "INSUFFICIENT_DATA", reasons
    return "PARITY_PASS_WU", ["all Opus parity criteria met"]
...
def render_markdown(report: dict) -> str:
    ...
    icao = report["icao"]
    lines = [
        f"# {city} Open-Meteo vs WU/{icao} Source Parity Audit",
        "",
        ...
        "## Objective",
        "",
        f"Compare {city} Open-Meteo proxy daily maximum temperature against the Polymarket settlement source, Weather Underground {icao}, before any Opus promotion review.",
        "",
        "## Sources Used",
        "",
        f"- City/ICAO: `{city}` / `{icao}`",
        f"- Open-Meteo archive: `temperature_2m_max`, lat `{report['lat']}`, lon `{report['lon']}`, timezone `{report['tz']}`",
        f"- Weather Underground settlement source: `{report['wu_url']}`",
        ...
        f"| max abs delta C | {metrics.get('max_abs_delta_c')} |",
        ...
        "| Criterion | Status |",
        "|---|---|",
        f"| WU n >= {OPUS_MIN_WU_N} | {'MET' if (metrics.get('n_compared') or 0) >= OPUS_MIN_WU_N else 'NOT_MET'} |",
        ...
        f"| max abs delta <= {OPUS_MAX_ABS_DELTA_C}C | {'MET' if metrics.get('max_abs_delta_c') is not None and metrics.get('max_abs_delta_c') <= OPUS_MAX_ABS_DELTA_C else 'NOT_MET'} |",
    ...
    lines.extend(
        [
            ...
            "",
            f"Provide a reliable WU/{icao} daily-high dataset or repo-approved WU fetcher, rerun this tool, and ask Opus to review the parity dossier only if the Opus criteria are met.",
        ]
    )
    return "\n".join(lines) + "\n"
...
def build_report(args) -> dict:
    ...
    icao = str(args.icao).strip().upper()
    if not city or not icao:
        raise SystemExit("--city and --icao are required")
    config = AuditConfig(
        city=city,
        icao=icao,
        lat=float(args.lat),
        lon=float(args.lon),
        tz=str(args.tz),
        wu_url=str(args.wu_url or f"https://www.wunderground.com/history/daily/{icao}"),
    )
    ...
    if args.open_meteo_csv:
        ...
        open_meteo = fetch_open_meteo_daily(start, end, config.lat, config.lon, config.tz)
    ...
    blocked_path = Path(args.blocked_jsonl) if args.blocked_jsonl else choose_existing(DEFAULT_BLOCKED_INPUTS)
    blocked_rows = parse_blocked_rows(load_jsonl(blocked_path), config.city) if blocked_path and blocked_path.exists() else []
    ...
    if getattr(args, "settlement_from_gamma", False):
        ...
        gamma_verdict = gamma_settlement.get("verdict")
        if gamma_verdict in {
            "SETTLEMENT_GAMMA_PARITY_FAIL",
            "SETTLEMENT_GAMMA_PARITY_PASS",
            "INSUFFICIENT_DATA",
        }:
            verdict = gamma_verdict
            reasons = list(gamma_settlement.get("verdict_reasons") or reasons)
...
def parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument("--city", required=True, help="City name as it appears in blocked_signals_resolutions.")
    parser.add_argument("--icao", required=True, help="Weather Underground settlement ICAO/station code.")
    parser.add_argument("--lat", required=True, type=float, help="Open-Meteo archive latitude.")
    parser.add_argument("--lon", required=True, type=float, help="Open-Meteo archive longitude.")
    parser.add_argument("--tz", required=True, help="Open-Meteo timezone, e.g. Asia/Shanghai.")
    parser.add_argument("--wu-url", help="Optional human-readable WU settlement URL for the dossier.")
    ...
    parser.add_argument("--wu-csv", help="Optional manual WU CSV with date,wu_high_c. No WU scraping is attempted.")
    parser.add_argument(
    ...
    parser.add_argument("--blocked-jsonl", "--blocked-resolutions", dest="blocked_jsonl", help="Path to blocked_signals_resolutions.jsonl.")
    parser.add_argument("--json-out")
    parser.add_argument("--out-md", "--md-out", dest="md_out")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_stem = f"{slugify_city(report['city']).replace('-', '_')}_open_meteo_vs_wu_parity"
    md_path = Path(args.md_out) if args.md_out else REPO_ROOT / "docs" / "source_audits" / f"{output_stem}.md"
    md_path.parent.mkdir(parents=True, exist_ok=True)
    md_path.write_text(md, encoding="utf-8", newline="\n")
    if not args.no_write_json:
        json_path = Path(args.json_out) if args.json_out else REPO_ROOT / "data" / "source_audits" / f"{output_stem}.json"
