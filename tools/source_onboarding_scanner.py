...
def _build_trader_stats(signals_rows):
    ...
    for row in signals_rows:
        ...
        is_range = "range" in condition or "between" in condition

        if source:
            by_city[city]["sources"].add(source)
        if day:
            by_city[city]["days"].add(day)
        if is_range:
            by_city[city]["range_count"] += 1
        by_city[city]["total_count"] += 1
    return dict(by_city)
...
def _build_blocked_stats(blocked_rows):
    ...
    for row in blocked_rows:
        ...
        if bot_eval is not None and str(bot_eval).strip():
            ...
            if outcome in ("win", "correct", "yes", "true", "1"):
                by_city[city]["wins"] += 1
...
def build_records(inputs, resolution_icao, degraded):
    ...
    all_signal_cities = set(trader_stats.keys()) | set(blocked_stats.keys())
...
def render_markdown(payload):
    ...
    lines += [
        ...
        "",
        "| City | State | Score | Feasibility | Traders (src/days) | Blocked WR (n) | Notes |",
        "| --- | --- | --- | --- | --- | --- | --- |",
    ]
    for r in payload["cities"]:
        t = r["trader"]
        b = r["blocked_signals"]
        wr_str = f"{b['wr']:.0%} (n={b['n']})" if b["wr"] is not None and b["qualifies"] else f"n={b['n']} (insufficient)"
        notes = []
        if r["trader"]["range_fraction"] >= RANGE_ONLY_THRESHOLD:
            notes.append("range_only")
        if r["source_feasibility"] == "no_icao":
            notes.append("no_icao")
        elif r["source_feasibility"] == "icao_only":
            notes.append("icao_only_no_station")
        notes_str = "; ".join(notes) if notes else "-"
        lines.append(
            f"| {r['city']} | {r['state']} | {r['priority_score']:.2f}"
            f" | {r['source_feasibility']} | {t['n_sources']}/{t['n_days']}"
            f" | {wr_str} | {notes_str} |"
        )
