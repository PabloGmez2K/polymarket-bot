...
|------|------|------------|---------------|---------|
| 2026-05-26 | Explícita | Sesión 394 | pending | Codex implementa Cohort Intelligence Loop v1 LOG_ONLY orientado a expansion monetizable. Nueva herramienta `tools/cohort_intelligence_report.py` reutiliza `bot_signal_evaluations`, `skip_log`, `blocked_signals_resolutions` y `trade_lifecycle` para calcular rendimiento por cohorte: exact/NO near-threshold, exact/NO far, directional NO y subcohortes directional NO por city/source/distance band. Emite metricas `n_closed`, W/L, WR, `avg_our_prob`, `calibration_gap`, PnL simulado unitario, PnL real no canonico si hay join, `last_seen`, `gate_current` y veredictos manuales. `tools/daily_bot_observability_run.py` lo integra en el Daily Bot Digest por defecto, fail-open y no accionable. `docs/cohort_intelligence_loop_v1.md` documenta inputs, consumer y trigger Opus. Runtime precheck confirmo deployment anterior `b12b6b74` SUCCESS y `RUNTIME_SHADOW_GATE_EXERCISED=YES` por fila Singapore exact/NO near-threshold en `skip_log` y `bot_signal_evaluations`. Validacion local parcial: 50 tests focales passed y `py_compile` OK. No se tocaron reglas live, BUY/SELL/SKIP, BANKROLL, sizing, guards, scheduler, whitelist, city modes, Fase C, accounting canonico ni env vars. |
