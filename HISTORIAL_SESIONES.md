...
|------|------|------------|---------------|---------|
| 2026-08-31 | Explicita | Sesion 428 - POLYMARKET_LAFABRICA_MR014_OS_MIGRATION_APPLY | `docs: adopt Lafabrica MR-014 operating system` | Sonnet DOCS_ONLY / A2 APPLY_LOCAL. Migración del sistema operativo documental desde adopción MR-003 hasta MR-014 (Lafábrica verificada `7c1c8bb133f5...`, `main`). Cobertura completa 65/65 `change_id` canónicos en `docs/meta/LAFABRICA_ADOPTION.md` reescrito con granularidad `change_id`. `PATTERN-06` reconciliado (agent_events.jsonl no era el ledger; `docs/meta/AGENT_EXPERIENCE_LEDGER.md` creado vacío). `PATTERN-14` con disposición de release terminal (`pending_critical: none` correcto); su gate operacional queda representado aparte, sin colapsar: `risk_live: unknown`, `activation_prerequisite: true`, `blocking_scope: NEXT_REAL_ORDER_WRITE`, `runtime_verification: pending`. Creados `docs/meta/ACTIVE_CONTEXT_PACK.md` (L0), `docs/meta/ACTIVE_DECISION_STATE.md`, `docs/orchestrator_chatgpt_project_instructions.md`. `ORCHESTRATOR.md`/`AGENTS.md` actualizados por delta: repo/handshake como autoridad, Outcome-First (A0-A3/R0-R3/Builder-Verifier-Closer), preflights `PATTERN-16`, bloque gestionado `UPDATE_NOTIFICATION_CHECK` MR-008. `CLAUDE.md` referencia `PROJECT_BOOTSTRAP.md`, conserva `OPERATIONS_PLAYBOOK.md`. `SKILL.md` context-bootstrap reordenado. Propuesta de Project Instructions previa marcada `SUPERSEDED`. `SLT-004` candidato abstracto registrado. `verify_before_deploy.py` 1259/1259 baseline y final, sin regresión. Sin trading, sin runtime, sin Railway, sin env vars, sin dependencias, sin tests tocados, sin push. STANDBY sin cambio. |
| 2026-06-25 | Explicita | E3_WR_PROVENANCE_CAPTURE_V0 | `feat(predictive): capture trader wr provenance forward-only` | Codex CODEMAP_FIRST / PATCH_IF_SAFE / RISK_CONTROL. Captura forward-only de provenance para `trader_win_rate`/`trader_historical_wr`: `signals.json` añade `signals_generated_at`, source, closed_positions_count, asof_method, cutoff `unknown` y leakage_status; BSR schema v3 copia esos campos a nuevas filas. Se documenta que `PRECONDITION_A` sigue bloqueada para filas historicas y solo futuras filas con provenance WR son auditables. Sin cambiar WR, senales, score, trading, BUY/SELL/SKIP, BANKROLL, Fase C, city modes, scheduler, env vars ni DB. Validado con focales, py_compile, git diff --check y verify_before_deploy.py 1259/1259. |
| 2026-06-20 | Explicita | SESSION_LEARNING_TRANSFER_AND_STANDBY_FLOW_SYNC | `docs(meta): install session learning transfer and standby flow` | Sonnet docs-only LITE. Instalación de SESSION_LEARNING_TRANSFER como estándar de transferencia metodológica saneada hacia lafabrica/Brain. Creación de docs/meta/SESSION_LEARNING_TRANSFER_QUEUE.md con candidatos SLT-001 (SHADOW_MODE_AND_KILL_SWITCH_DISCIPLINE), SLT-002 (LONG_RUNNING_AGENT_PROJECT_GOVERNANCE), SLT-003 (STANDBY_AS_FIRST_CLASS_PROJECT_STATE). Actualización de AGENTS.md (bloque SLT opcional en Cierre con reglas de privacidad polymarket), ORCHESTRATOR.md (sección 15 STANDBY_AND_LEARNING_TRANSFER_FLOW), CONTEXTO.md (entrada append) e HISTORIAL_SESIONES.md. Sincronización de flujo con lafabrica-template (CHILD_PROJECT_TRANSFER_PROTOCOL, DEC-LAF-17), bijuymoda-suite y pablo-operating-brain (PRIVATE_DATA_BOUNDARIES, REPO_IMPORT_POLICY). STANDBY confirmado. Sin código, sin Railway, sin env vars, sin bot.py, sin trading, sin BANKROLL, sin Fase C, sin city modes, sin guards, sin scheduler, sin runtime, sin DB. |
| 2026-06-10 | Explicita | Sesion 424 | env var Railway + docs | Fable FULL RISK_CONTROL. VERIFY_AND_LOCK_STANDBY_KILL_SWITCH: la auditoria read-only previa (STANDBY_EXECUTION_GATE_AUDIT) concluyo STANDBY_ALERTS_ONLY — OPERATIONAL_PHASE=STANDBY no bloquea BUY real (camino allowlisted→selected→execute_trade nunca lee la fase). Verificacion filtrada Railway: SHADOW_ONLY_MODE=false explicito (hueco abierto: _is_shadow_only() retorna False directo, sin fallback legacy; vector autopromocion auto_canary, precedente Seoul 2026-05-20). Con autorizacion literal condicional de Pablo: set SHADOW_ONLY_MODE=true via railway_safe.ps1; redeploy SUCCESS (2026-06-10T06:20Z); re-verificado true en variables y printenv del contenedor; smoke read-only: bot vivo v10.6.50, ciclo 502 sin exposicion (management 0/0/0, buys=[]). Resultado: BUY real bloqueado global (gate shadow_only_override); SELL/gestion, resolver y collectors/accrual E3 intactos. Sin codigo, sin bot.py, sin DB, sin BANKROLL, sin Fase C, sin city modes, sin sizing, sin scheduler. Pendiente no urgente: STANDBY_EXECUTION_GATE_V0 (defensa en profundidad, otra sesion). |
| 2026-06-09 | Explicita | Sesion 423 | `feat: standby alarm hygiene v0 (OPERATIONAL_PHASE)` | Fable CODE NORMAL. STANDBY_ALARM_HYGIENE_V0: alinear alertas/digest con Bot Stable v0 STANDBY tras cierre Phase 2. Nueva constante OPERATIONAL_PHASE (default STANDBY en codigo, override env var = FULL). Gates: phase2 monitor silenciado (fase muerta), v2 expansion trigger desactivado en STANDBY (footgun RECALIBRATION_PHASE2_CLOSED cableado a sugerencia de expansion), bankroll scaling sin resumen periodico (transiciones se mantienen). Banner STANDBY en digest diario con next trigger E3 2026-06-23. RISK_KEEP intactos: signals health, recorder, low bankroll, city accuracy, canary/active candidates. TRIGGER_WATCH intactos: crosscheck, traders intelligence, blocked signals (accrual E3). Docs: docs/standby_alarm_hygiene_v0.md. Tests 45/45 focal + caso STANDBY en verify. Sin BUY/SELL/SKIP, BANKROLL, Fase C, city modes, exact/NO, sizing, env vars. |
| 2026-06-09 | Explicita | Sesion 422 | `docs: close Phase 2 T30 stop-current-line decision` | Sonnet docs-only. Cierre durable Phase 2 T+30. Veredicto Fable: STOP_CURRENT_LINE / PHASE2_CLOSED_FAIL_B6_ZERO_THROUGHPUT_TAIL. Readout Railway SSH (Fable): 36 cierres PnL / 37 filas; B5 PASS n=36>=25; WR 47.2% (17W/19L); P&L +$2.11 nc / ex-outlier Wellington -$16.94; B6 FAIL solo Tokyo n=3 WR>=40%; BA n=0; Ankara n=0; 6/37 trades de ciudades ACTIVE; 62% exact/NO bloqueados; micro_position_unsellable 11 cierres -$22.77; 0 trades desde 2026-05-26. Funnel ciclos 497-499: with_edge=0; condition_filtered mata 100%. Invariantes: BANKROLL HOLD $25; Fase C no autorizada; BUY/SELL/SKIP sin cambio; env vars/city modes sin cambio. Triggers: A) trader-following check 2026-06-23 (rerun trader_benchmark.py cuando celda forward n>=10 top1<=50%); B) Forecast path si E2 forward_holdout n>=30; C) exact/NO criterio S418. Sin codigo, sin Railway, sin DB, sin env vars, sin bot.py. |
| 2026-06-06 | Explicita | Sesion 421 | docs-only | Codex LITE docs-only. Se registra el Executable-Subset Payoff Test pedido por Opus como experimento manual/paper previo a redisenar policy o matar weather. Resultado read-only: BSE tenia 12 eval_keys forward con precio bot-side, pero 0 outcome local usable; E3 tenia forward_n=129 solo en summary agregado/sanitizado, sin row-level para payoff/mediana/ex-top1; n usable final=0. Veredicto FAIL / DATA_GAP-AS-FAIL. Implicacion: no hay base defendible para REDESIGN_POLICY_PAPER_ONLY ahora; gate 09-jun debe considerar Phase 2 + este FAIL, probable D-preliminary salvo contradiccion fuerte. Sin codigo, sin Railway, sin env vars, sin DB real, sin bot.py/trading core/city modes/BANKROLL/Fase C, sin push/deploy. |
| 2026-06-03 | Explicita | Sesion 420 | docs-only | Sonnet docs-only. Cierre ALPHA_SOURCE_PIVOT_STRATEGY_V1 emitido por Opus. Veredicto: START_ALPHA_PIVOT_SPRINT. Evidence frozen predictor negativa (L0 n=41 brier_adv=-0.0657; at_or_above|YES n=20 brier_adv=-0.1017). E1/E2 congelados hasta rerun trigger. Camino A primario: Trader-Following Benchmark (BSR/blocked_signals_resolutions). Camino B secundario: Forecast Path Autopsy source-fidelity 1p. Antes del 09-jun: no microstructure/H3/exact-NO/dashboards/city-mode como alpha. Phase 2 sigue carril canonico live. BANKROLL HOLD $25. Fase C no autorizada. No live, no env vars, no Railway, no codigo, no push. |
| 2026-06-03 | Explicita | Sesion 419 | `ce017c3` | Sonnet docs-only. Cierre LITE de E2 Predictor Benchmark implementado por Codex como runner offline/read-only. Artefactos: `tools/predictor_benchmark.py`, `tests/test_predictor_benchmark.py`, `data/predictive/benchmark_summary.json`; commit local `ce017c3 feat(predictive): add predictor benchmark protocol runner`. Estado E2: `benchmark_input_rows=41`, `global_verdict=INSUFFICIENT_N`, L0 forward `n=0`; L1 forward `at_or_above|YES n=0 INSUFFICIENT_N`, `at_or_below|YES n=0 INSUFFICIENT_N`, `exact|NO NON_PROMOTABLE_BY_POLICY`. Evidence frozen negativo: E2 `at_or_above|YES n=20 brier_advantage=-0.101739`; E1 builder sigue reproduciendo `at_or_above brier_advantage=-0.0939`. Exact/NO firewall aplicado; no hay candidato canary ni autorizacion de trading/BANKROLL/Fase C/env vars/city modes. Trigger de rerun: alguna L1 `forward_holdout n>=30` o revision del 09-jun si Phase 2 lo requiere. Sin codigo en esta sesion, sin Railway, sin deploy, sin DB writes, sin push. |
...
| 2026-05-26 | Explícita | Sesión 397 | pending | Codex implementa captura/enlace forward LOG_ONLY para que nuevas señales `directional NO` puedan producir outcomes calibration-safe. Identidad real: `bot_signal_evaluations.eval_key` == `signals.json`/resolver `match_key` (`city|date|condition|threshold|unit`), con enrichment de cierre Polymarket/Gamma (`condition_id`, `market_id`, tokens, slug) en `blocked_signals_resolutions.jsonl`. `bot.py` solo amplia el resolver diario LOG_ONLY para incluir `condition in {at_or_above, at_or_below}`, `outcome=No`, con `match_key` real y fecha cerrable; no toca admisión live ni gates. `tools/cohort_intelligence_report.py` fija `forward_capture_start=2026-05-26T16:15:11Z`, separa métricas forward (`directional_forward_seen`, `directional_forward_resolved_calibration_unique`, status) y limita gates de promoción directional a outcomes forward enlazados. `tools/daily_bot_observability_run.py` muestra el estado forward; docs actualizadas. No backfill heurístico de los 18 trades históricos; `trade_lifecycle` sigue solo P&L no canónico. No se tocan `SHADOW_EXACT_NO_GLOBAL`, `PAUSE_WELLINGTON_EXACT_NO`, directional NO live, BANKROLL, sizing, guards, scheduler, whitelist, city modes, Fase C, accounting canónico, env vars, Telegram ni BUY/SELL/SKIP. |
| 2026-05-26 | Explícita | Sesión 396 | pending | Codex corrige la unidad de analisis del Cohort Intelligence Loop v1 LOG_ONLY. `tools/cohort_intelligence_report.py` separa `raw_signals`, `resolved_market_calibration` y `executed_trade_pnl`; deduplica calibracion por identidad de mercado resuelto + side + outcome usando primero `eval_key`/`match_key` (`city|date|condition|threshold|unit`), con `condition_id` y `market_id` solo como fallback porque pueden agrupar multiples strikes en weather. Calcula P&L real reportado no canonico solo desde ejecuciones identificables de `trade_lifecycle` (`id` > `position_key` + anchor temporal). Los veredictos pasan a usar `n_closed_calibration_unique`, WR/gap de calibracion y PnL simulado unitario; faltas de identidad degradan a `DATA_QUALITY_BLOCKER`. `tools/daily_bot_observability_run.py` muestra raw/calibration/executed en el digest y conserva fail-open. `docs/cohort_intelligence_loop_v1.md` documenta que los primeros counts exact/NO eran diagnosticos raw/no calibration-safe. Tests focales cubren dedup de reevaluaciones, reentradas reales del mismo mercado, directional NO limpio, veredictos con counts unicos y degradacion segura. No se tocan gates live, `bot.py`, BANKROLL, sizing, guards, scheduler, whitelist, city modes, Fase C, accounting canonico, env vars, Telegram ni BUY/SELL/SKIP. |
| 2026-05-26 | Explícita | Sesión 395 | pending | Codex aplica FULL limitado autorizado para extender el gate SHADOW_ONLY de exact/NO a todo `condition=exact`, `side=NO`, eliminando la restricción de distancia al target. `bot.py` añade `SHADOW_EXACT_NO_GLOBAL`, `_is_exact_no_global_shadowed()` y `_exact_no_distance_telemetry()`; el gate conserva los patrones existentes de `shadow_trades`, `skip_log` (`shadow_only_override`) y `bot_signal_evaluation` (`would_buy=False`, `decision_gate=SHADOW_EXACT_NO_GLOBAL`), manteniendo telemetría near/far cuando forecast/target son parseables. `PAUSE_WELLINGTON_EXACT_NO` sigue ganando primero como redundancia temporal. `tools/cohort_intelligence_report.py` reconoce el nuevo gate global, sin tocar directional NO ni convertir el reporte LOG_ONLY en acción automática. Tests focales cubren exact/NO near/far, parse failures, no captura YES/range/otras condiciones y orden de gates. No se tocan directional NO, BANKROLL, sizing, guards, scheduler, whitelist, city modes, Fase C, accounting, env vars ni Telegram. |
| 2026-05-26 | Explícita | Sesión 394 | `ba6e0d1` | Codex implementa Cohort Intelligence Loop v1 LOG_ONLY orientado a expansion monetizable. Nueva herramienta `tools/cohort_intelligence_report.py` reutiliza `bot_signal_evaluations`, `skip_log`, `blocked_signals_resolutions` y `trade_lifecycle` para calcular rendimiento por cohorte: exact/NO near-threshold, exact/NO far, directional NO y subcohortes directional NO por city/source/distance band. Emite metricas `n_closed`, W/L, WR, `avg_our_prob`, `calibration_gap`, PnL simulado unitario, PnL real no canonico si hay join, `last_seen`, `gate_current` y veredictos manuales. `tools/daily_bot_observability_run.py` lo integra en el Daily Bot Digest por defecto, fail-open y no accionable. `docs/cohort_intelligence_loop_v1.md` documenta inputs, consumer y trigger Opus. Runtime precheck confirmo deployment anterior `b12b6b74` SUCCESS y `RUNTIME_SHADOW_GATE_EXERCISED=YES`; deploy final Railway `95635bc8-dd6f-47f5-9af3-1a7da66988f0` SUCCESS. Smoke runtime: exact/NO near-threshold `REVIEW_BLOCK_LIVE`; exact/NO far `REVIEW_BLOCK_LIVE`; directional NO `n_closed=18`, 10W/8L, WR 55.6%, PnL real no canonico +4.58, `REVIEW_OPUS`, sin candidata canary. Validaciones: 52 tests focales, `py_compile`, `git diff --check`, `verify_before_deploy.py` 1255/1255. No se tocaron reglas live, BUY/SELL/SKIP, BANKROLL, sizing, guards, scheduler, whitelist, city modes, Fase C, accounting canonico ni env vars. |
| 2026-05-25 | Explícita | Sesión 393 | `00338af` | Codex implementa FULL limitado autorizado por Pablo para mover a SHADOW_ONLY nuevas señales `exact/NO` near-threshold (`abs(forecast-target) < 1.5°C`, equivalente Fahrenheit para mercados °F). Patch acotado en `bot.py`: `SHADOW_EXACT_NO_NEAR_THRESHOLD`, helper `_is_exact_no_near_threshold_shadowed()` y gate tras edge/min-edge/Kelly positivo y tras scaling virtual, antes de allowlist/BUY append. Wellington mantiene prioridad con `PAUSE_WELLINGTON_EXACT_NO`; el nuevo scope registra `shadow_trades`, `skip_log` con `skip_reason=shadow_only_override` y extras de evidencia, y `record_bot_evaluation(... would_buy=False, decision_gate=SHADOW_EXACT_NO_NEAR_THRESHOLD)`. Test focal ampliado cubre Celsius, Fahrenheit, no captura YES/range/rangos/missing values y orden de gates. Validaciones: focales 6 passed, `py_compile bot.py` OK, `git diff --check` OK, `verify_before_deploy.py` 1255/1255. Railway deployment `e452bb7b-5a83-40ea-9c46-453846aa7dc7` SUCCESS; smoke read-only confirma IDs en `/app/bot.py`, bot REAL v10.6.50 y wallet `positions_count_open=0`, `active_positions_value=0.0`. No se tocan BANKROLL, sizing, guards, scheduler, whitelist, city modes, accounting, Fase C, env vars ni alertas Telegram nuevas. |
| 2026-05-25 | Explícita | Sesión 392 | `92af499` | Codex implementa FULL limitado de contención Wellington exact/NO tras confirmación de Pablo de la segunda venta manual de Wellington 16°C May26 NO (4.5 acciones a 44¢, +$1.98). Patch acotado en `bot.py`: helper `_is_wellington_exact_no_paused()`, constante `PAUSE_WELLINGTON_EXACT_NO` y gate después de edge/min-edge/Kelly positivo pero antes de scaling/BUY append. El gate bloquea solo `city=Wellington`, `condition=exact`, `side=NO`, registra `skip_reason="cohort_paused"` con `cohort_pause_id` en `skip_log` y `record_bot_evaluation(... would_buy=False, decision_gate=PAUSE_WELLINGTON_EXACT_NO)`. Test focal nuevo cubre Wellington exact NO bloqueado y YES/range/otras ciudades no bloqueadas. Validaciones: focales 9 passed, `py_compile`, `git diff --check`, `verify_before_deploy.py` 1255/1255. Railway code deployment `abaccd9a-4b51-4480-a5cb-99e7e44a0e16` SUCCESS; smoke read-only confirma `PAUSE_WELLINGTON_EXACT_NO` en `/app/bot.py`, bot REAL v10.6.50, wallet `positions_count_open=0`. Sin env var nueva; no se toca BANKROLL, sizing, guards, scheduler, otras ciudades, otras condiciones, accounting, NOAA, whitelist, city modes, SL, DB ni Fase C. |
