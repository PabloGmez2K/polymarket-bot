...
|------|------|------------|---------------|---------|
| 2026-05-25 | Explícita | Sesión 393 | pending | Codex implementa FULL limitado autorizado por Pablo para mover a SHADOW_ONLY nuevas señales `exact/NO` near-threshold (`abs(forecast-target) < 1.5°C`, equivalente Fahrenheit para mercados °F). Patch acotado en `bot.py`: `SHADOW_EXACT_NO_NEAR_THRESHOLD`, helper `_is_exact_no_near_threshold_shadowed()` y gate tras edge/min-edge/Kelly positivo y tras scaling virtual, antes de allowlist/BUY append. Wellington mantiene prioridad con `PAUSE_WELLINGTON_EXACT_NO`; el nuevo scope registra `shadow_trades`, `skip_log` con `skip_reason=shadow_only_override` y extras de evidencia, y `record_bot_evaluation(... would_buy=False, decision_gate=SHADOW_EXACT_NO_NEAR_THRESHOLD)`. Test focal ampliado cubre Celsius, Fahrenheit, no captura YES/range/rangos/missing values y orden de gates. Validaciones predeploy: focales 6 passed, `py_compile bot.py` OK, `verify_before_deploy.py` 1255/1255. No se tocan BANKROLL, sizing, guards, scheduler, whitelist, city modes, accounting, Fase C, env vars ni alertas Telegram nuevas. Railway deploy pendiente en esta sesión. |
