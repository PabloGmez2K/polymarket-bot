...
**Última actualización:** 21 de abril de 2026 (Sesión 215 — P4 whitelist expansion v10.6.27: Tel Aviv, Taipei, Singapore, Wuhan)
**Sesión 215 (21 abr 2026, Sonnet):** P4 whitelist expansion del canary exact/range. Tras confirmar que el checkpoint día 7 (WR=40% n=5) es `OK_INSUFICIENTE` (n<15, no es close trigger; el close trigger requiere n≥15 y WR<50%), se identificaron 4 ciudades ya en `RESOLUTION_STATIONS` con alta WR en blocked signals que no estaban en `QUALITY_TRADER_CITIES_WHITELIST`. Cambios en `bot.py` v10.6.27: (1) **Tel Aviv** añadida (3/3=100% blocked WR, NOAA verificado LLBG, ya en `OBSERVED_AUDIT_CITIES`); (2) **Taipei** añadida (3/3=100% blocked WR, ICAO-only RCTP); (3) **Singapore** añadida (TRADER_ONLY 2/2, ICAO-only WSSS); (4) **Wuhan** añadida (TRADER_ONLY 2/2, ICAO-only ZHHH). Además se corrigió bug en el prompt de `maybe_alert_p4_p5_expansion`: la precondición original abortaría con WR<50% aunque n=5<15, confundiendo muestra insuficiente con close trigger. La precondición ahora distingue `OK_INSUFICIENTE` (n<15 → continuar) de `CLOSE/ALERT` real (n≥15 y WR<50%). `verify_before_deploy.py` pasa **749/749** (3 tests nuevos v10.6.27). **Pendiente Railway**: actualizar env var `QUALITY_TRADER_CITIES_WHITELIST` añadiendo `,Tel Aviv,Taipei,Singapore,Wuhan` al final. **P5 pendiente** (Amsterdam, Moscow, Jeddah, Istanbul, Helsinki — requieren RESOLUTION_STATIONS nuevas): análisis Opus en curso, aplicar en sesión siguiente.
