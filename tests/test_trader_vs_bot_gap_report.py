...
Tests cover:
  - Classification logic (3 dimensions: win_for_trader × would_buy × exec_status)
  - actual_execution_status derivation from city_mode + reason_blocked
  - blocking_gate derivation
  - join logic (BSE ↔ BSR via eval_key/match_key)
  - data_quality_flags
  - gates_passed / gates_missing
...
class TestClassifyRow:
    ...
    def test_no_action_trader_won_bot_would_buy_but_not_blocked(self):
        # exec_status FILTERED_NO_EDGE + would_buy=True is contradictory; fall to NO_ACTION
...
class TestGatesStatus:
    ...
    def test_gate5_fails_no_bse(self):
        row = self._row()
...
class TestBuildReport:
    ...
    def test_v1_row_is_no_action(self):
        bse_p, bsr_p, scx_p = self._paths()
        _write_jsonl(bse_p, [])
        _write_jsonl(bsr_p, [BSR_V1_ROW])
        _write_jsonl(scx_p, [])
        report = build_report(self.tmp)
    ...
    def test_v3_row_shadow_no_bse_is_no_action(self):
        """BSR v3 shadow + win, but no BSE match → would_buy unknown → NO_ACTION."""
        bse_p, bsr_p, scx_p = self._paths()
        _write_jsonl(bse_p, [])
        _write_jsonl(bsr_p, [BSR_V3_ROW])
        _write_jsonl(scx_p, [])
        report = build_report(self.tmp)
        row = report["rows"][0]
        assert row["actual_execution_status"] == "SHADOW_ONLY"
        assert row["bot_would_buy"] is None
        assert row["learning_classification"] == "NO_ACTION_INSUFFICIENT_EVIDENCE"
        assert "bot_would_buy_unknown" in row["data_quality_flags"]
    ...
    def test_v3_row_with_bse_join_candidate_filtered_winner(self):
        ...
        report = build_report(self.tmp)
        row = report["rows"][0]
        assert row["join_confidence"] == "MEDIUM"
        ...
        assert "gates_passed" in row
        assert "counterfactual_estimable" in row["gates_passed"]

    def test_candidate_missed_opportunity_blocked(self):
        """would_buy=True + win_for_trader=True + BLOCKED_POLICY → CANDIDATE_MISSED_OPPORTUNITY."""
        bse_p, bsr_p, scx_p = self._paths()
        bse_row = dict(BSE_ROW)
        bse_row["would_buy"] = True
        bse_row["decision_gate"] = None
        bse_row["skip_or_block_reason"] = None
        bsr_row = dict(BSR_V3_ROW)
    ...
    def test_protective_policy_evidence(self):
        """would_buy=True + win_for_trader=False + SHADOW_ONLY → PROTECTIVE_POLICY_EVIDENCE."""
        bse_p, bsr_p, scx_p = self._paths()
        bse_row = dict(BSE_ROW)
        bse_row["would_buy"] = True
        bse_row["decision_gate"] = None
    ...
    def test_summary_counts(self):
        bse_p, bsr_p, scx_p = self._paths()
        _write_jsonl(bse_p, [BSE_ROW])
        _write_jsonl(bsr_p, [BSR_V3_ROW, BSR_V1_ROW])
        _write_jsonl(scx_p, [])
        report = build_report(self.tmp)
        s = report["summary"]
        assert s["total_bsr_rows"] == 2
        assert s["bse_eval_keys_indexed"] == 1
        assert s["joined_rows"] == 1  # only BSR_V3_ROW matches BSE
